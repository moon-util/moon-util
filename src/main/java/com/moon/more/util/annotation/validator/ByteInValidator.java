package com.moon.more.util.annotation.validator;

import com.moon.more.util.annotation.ByteIn;

import javax.validation.ConstraintValidator;
import javax.validation.ConstraintValidatorContext;

/**
 * @author benshaoye
 */
public class ByteInValidator extends BaseValidator implements ConstraintValidator<ByteIn, Object> {

    public ByteInValidator() { super(hashSet()); }

    @Override
    public void initialize(ByteIn annotation) {
        initialArgs(annotation.nullable(), annotation.values(), Byte::valueOf);
    }

    @Override
    public boolean isValid(Object value, ConstraintValidatorContext context) { return doValidateValue(value); }

    @Override
    protected Class getTargetClass() { return ByteIn.class; }

    @Override
    protected boolean afterPreTransformTest(String value) { return NUMERIC.test(value); }
}
