 ...
 * -
 * -       // 创建一个命名工作簿，如果存在指定名称工作簿就直接使用已存在的，否则就新建
 * -       // 如果是已存在的 sheet，还可以传入 append 参数，表示数据默认是追加还是覆盖
 * -       // 默认 append = true；
 * -       excelFactory.sheet("sheetName", sheetFactory -> {
 * -
 * -           // 设置列宽，按索引设置列宽，忽略 null，可用 null 来跳过设置某些列
 * -           sheetFactory.setColumnsWidth(3000,4000,null,5000,6000);
 * -
 * -           // 定义样式，可以在任意位置定义样式，并在任意位置使用
 * -           // 由于 excel 有限制 CellStyle 的数量上限，所以建议通过这“定义+引用”的方式使用
 * -           // 尤其不要在循环里创建 CellStyle，
 * -           sheetFactory.definitionStyle("unique-classname", (style, font) -> {
 * -
 ...
 * -
 * -           // 预定义一个注释
 * -           sheetFactory.definitionComment("defined-comment-unique-name", "预定义注释");
 * -
 * -           // 可以通过链式调用创建“行”和“单元格”
 ...
 * -       // 实际你要应用你所设置的样式和注释的话，需要手动调用 finish 方法才最后应用那些样式和注释
 * -       }).finish().write(...);
...
public final class ExcelUtil {
     ...
     * @return Excel 操作工厂（代理）
     */
    public static ExcelFactory stream() { return of(SUPER); }
