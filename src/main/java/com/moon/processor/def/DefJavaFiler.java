...
public class DefJavaFiler implements JavaFileWriteable {
    ...
    public String toString() {
        ...
        if (getType() == Type.ENUM) {
            ...
            for (String anEnum : getEnums()) {
                addr.next().indent(4).append(Holder.name.on(template, anEnum));
            }
        }
        ...
        StringAddr constAddr = new StringAddr();

        // const of custom
        if (!constantsMap.isEmpty()) {
            for (String value : constantsMap.values()) {
                constAddr.next(2).indent(4).addScript(value);
            }
        }
        ...
        if (scripts != null && scripts.length > 0) {
            ...
            for (String script : scripts) {
                constAddr.next(2).indent(4).addScript(script);
            }
        }
        ...
        // static block
        buildBlock("static {", 4, addr, staticBlock);

        // static block
        buildBlock("{", 4, addr, instanceBlock);
    ...
    private static void buildBlock(
        String blockStarting, int indented, StringAddr addr, Map<String, String> block
    ) {
        List<String> scripts = new ArrayList<>(block.values());
        if (!scripts.isEmpty()) {
            addr.next(2).indent(indented).add(blockStarting);
            for (String value : scripts) {
                addr.next().indent(indented).indent(indented).addScript(value);
            }
            addr.next().indent(indented).add('}');
        }
    }

    private final static LocalDateTime NOW = LocalDateTime.now();

    private void autoGenerated(StringAddr addr, int indented) {
        ...
        if (Imported.GENERATED) {
            ...
            addr.next().indent(indented).add("value = \"").add(author).add("\",");
            addr.next().indent(indented).add("date = \"").add(NOW).add("\"");
            addr.next().add(')');
        }
