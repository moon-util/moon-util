...
public final class EncryptUtil {
    ...
    public final static String sha1(String input) {
        return SHA_1.encrypt(input);
    }

    public final static String sha256(String input) {
        return SHA_256.encrypt(input);
    }

    public final static String sha384(String input) {
        return SHA_384.encrypt(input);
    }

    public final static String sha512(String input) {
        return SHA_512.encrypt(input);
    }

    public final static String encrypt(String input, EncryptType... types) {
        for (int i = 0; i < types.length; i++) {
            input = types[i].encrypt(input);
        }
        return input;
    }
