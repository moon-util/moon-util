...
class ParseDelimitersTestTest {
    ...
    void testParse() {
        str = "本草纲目";
        handler = running(str);
        REQUIRES.requireInstanceOf(handler, DataStr.class);
        REQUIRES.requireEquals(handler.run(), str);

        str = "本草纲目{{'好的'}}";
        handler = running(str);
        REQUIRES.requireInstanceOf(handler, DataStr.class);
        REQUIRES.requireEquals(handler.run(), "本草纲目好的");

        str = "本草纲目{{'好的'}}  {{123}}";
        handler = running(str);
        REQUIRES.requireInstanceOf(handler, DataStr.class);
        REQUIRES.requireEquals(handler.run(), "本草纲目好的  123");
        ...
        handler = running(str);
        REQUIRES.requireInstanceOf(handler, DataStr.class);
        REQUIRES.requireEquals(handler.run(), "本草纲目好的  123  电脑 17");
    }
    ...
    void testParseGetter() {
        ...
        REQUIRES.requireInstanceOf(handler, AsGetter.class);
        REQUIRES.requireEquals(handler.run(data), "本草纲目好的  123  电脑 17 456");

        MapUtil.putToObject(data, "name", "就问你嗨不嗨");
        MapUtil.putToObject(data, "true", "你是不是傻");

        REQUIRES.requireEquals(handler.run(data), "本草纲目好的  123  电脑 17 就问你嗨不嗨");
        REQUIRES.requireEquals(handler1.run(data), "本草纲目好的  123你是不是傻  电脑 17 就问你嗨不嗨");
    }
    ...
    void testParseGetter1() {
        ...
        handler1 = running(str1, delimiters);
        REQUIRES.requireEquals(handler1.run(data), "本草纲目好的  123你是不是傻 回长沙true  电脑 17 就问你嗨不嗨");

        str1 = "本草纲目${'好的'}}  ${123}}${['true']}} ${[true]+true+name}}  电脑 ${1+2+3+5+6}} ${name}}";
        handler1 = running(str1, delimiters);
        str = "本草纲目好的  123你是不是傻 回长沙true就问你嗨不嗨  电脑 17 就问你嗨不嗨";
        REQUIRES.requireEquals(handler1.run(data), str);
    }
    ...
    void testDelimiters() {
        ...
        REQUIRES.requireEquals(runner.run(data), "今天我要去 北京 上班，下午 5 点下班");
